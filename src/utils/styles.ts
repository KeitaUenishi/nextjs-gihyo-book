...
// keyof typeof解説 https://zenn.dev/harryduck/articles/9d09b1c133f9cd
...
// (string & {}) https://nanto.asablo.jp/blog/2021/09/11/9422241
// 各Themeのキーの型
  ...
  if (isResponsivePropType(prop)) {
    ...
    for (const responsiveKey in prop) {
      if (responsiveKey === "base") {
        ...
        const style = `${propKey}: ${toThemeValueIfNeed(
          propKey,
          prop[responsiveKey],
          theme,
        )};`;
...
function toThemeValueIfNeed<T>(propKey: string, value: T, theme?: AppTheme) {
  if (
    theme &&
    theme.space &&
